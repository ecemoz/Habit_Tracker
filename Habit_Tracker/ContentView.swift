            ...
            VStack { //Üst üste koyduğum parçalar için
                HStack {
                    TextField("Yeni alışkanlık ekle", text: $newHabitTitle)
                        ...
                        //Kullanıcımın yazı girebildiği alan // $ bu ile buraya kullanıcım birşey yazdığında ben direkt onu newHabitTitle değişkenine bağlamak istedim.
                    ...
                    .foregroundStyle(Color("ArticleColor")) // Buton başlık rengi (metin)
                    .buttonStyle(.borderedProminent)
                    .tint(Color("AccentColorPinkBlue")) // Buton arka plan rengi
                }
                .padding()
                ...
                List {
                    ForEach(habits) { habit in //satıra tıklayınca  alışkanlık detay ekranına gitmesini istiyorum- navigation link ile
                        NavigationLink(destination: HabitDetailView(habit: binding(for: habit), onSave: saveHabits)) {
